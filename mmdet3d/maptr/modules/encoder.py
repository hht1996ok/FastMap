...
class BaseTransform(BaseModule):
    ...
    def forward(
        ...
        for img_meta in img_metas:
            ...
            lidar_aug_matrix.append(img_meta['lidar_aug_matrix'])
        ...
        lidar_aug_matrix = images.new_tensor(lidar_aug_matrix)  # (B, N, 4, 4)
        ...
        if use_depth_loss:
            x, depth, depth_pre = self.get_cam_feats(images, mlp_input)
            x = self.bev_pool(geom, x)
            # import pdb;pdb.set_trace()
            x = x.permute(0, 1, 3, 2).contiguous()
            return x, depth, depth_pre
        else:
...
class LSSTransform(BaseTransform):
    def __init__(
        ...
        if self.use_depth_loss:
            self.upsample_depth = nn.Sequential(
                nn.ConvTranspose2d(in_channels, out_channels, kernel_size=5, padding=2, stride=2, output_padding=1, bias=False),
                nn.BatchNorm2d(out_channels),
                nn.ReLU(),
                nn.ConvTranspose2d(out_channels, out_channels, kernel_size=5, padding=2, stride=2, output_padding=1, bias=False),
                nn.BatchNorm2d(out_channels),
                nn.ReLU(),
                nn.ConvTranspose2d(out_channels, out_channels, kernel_size=5, padding=2, stride=2, output_padding=1, bias=False),
                nn.BatchNorm2d(out_channels),
                nn.ReLU(),
                nn.Conv2d(out_channels, self.D, kernel_size=1),
            )
    ...
    def forward(self, images, img_metas):
        if self.use_depth_loss:
            x, depth, depth_pre = super().forward(images, img_metas, self.use_depth_loss)
            x = self.downsample(x)
            ret_dict = dict(
                bev=x,
                depth=depth,
            ...
            x, depth = super().forward(images, img_metas, self.use_depth_loss)
            x = self.downsample(x)
            ret_dict = dict(
                bev=x,
                depth=depth,
    ...
    def get_depth_loss(self, depth_labels, depth_preds):
        # import pdb;pdb.set_trace()
        if self.use_depth_loss:
            depth_label_size = depth_labels.size()
            depth_labels = depth_labels.reshape(depth_label_size[0] * depth_label_size[1], depth_label_size[2], depth_label_size[3])  # 6, H, W
            depth_list = []
            for depth in depth_labels:
                depth = torch.nn.functional.interpolate(depth.unsqueeze(0).unsqueeze(0), scale_factor=1/self.feat_down_sample).squeeze(0)
                depth_list.append(depth)
            depth_labels = torch.cat(depth_list, dim=0).reshape(-1)
            mask = torch.nonzero((depth_labels >= 1.0) * (depth_labels < 35.0)).squeeze(1)
            depth_labels = depth_labels[mask]

            depth_preds = depth_preds.permute(0, 2, 3, 1).reshape(-1, 68)[mask, :]
            depth_labels = (depth_labels - 1.0) // 0.5
            loss_predict_depth = self.loss_predict_depth(depth_preds, depth_labels.long(), avg_factor=mask.size(0))
            return self.loss_depth_weight * loss_predict_depth
        else:
            if depth_preds is None:
                return 0
            depth_labels = self.get_downsampled_gt_depth(depth_labels)
    ...
    #     """
    #     B, N, H, W = gt_depths.shape
    #     gt_depths = F.interpolate(gt_depths, scale_factor=1 / self.feat_down_sample, mode='bilinear', align_corners=False).view(-1, H // self.feat_down_sample,
    #                                                                                                                         W // self.feat_down_sample)
    #     # gt_depths = gt_depths.view(B * N, H // self.feat_down_sample,
    #     #                            self.feat_down_sample, W // self.feat_down_sample,
    #     #                            self.feat_down_sample, 1)
    ...
    #     gt_depths = torch.where((gt_depths < self.D + 1) & (gt_depths >= 1.0), gt_depths, torch.zeros_like(gt_depths))
    #     gt_depths = F.one_hot(gt_depths.long(), num_classes=self.D).view(-1, self.D)
    #     return gt_depths.float()
    ...
    # #         return 0
    # #     depth_labels = self.get_downsampled_gt_depth(depth_labels)
    # #     depth_preds = depth_preds.permute(0, 2, 3, 1).reshape(-1, self.D).contiguous()
    # #     #import pdb; pdb.set_trace()
    # #     fg_mask = torch.max(depth_labels, dim=1).values > 0.0 # 只计算有深度的前景的深度loss
    # #     import pdb; pdb.set_trace()
    # #     #fg_mask = depth_labels > 0.0  # 只计算有深度的前景的深度loss
