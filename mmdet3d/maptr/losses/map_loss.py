...
def diff_pts_l1_loss(pred, target, cls_scores, labels, weight, reduction, avg_factor):
    instance_num, pts_num, _ = pred.size()
    if target.numel() == 0:
        return pred.sum() * 0
    assert pred.size() == target.size()
    #import pdb; pdb.set_trace()
    vector_dic_v1 = (pred[:, :-1, :] - pred[:, 1:, :]) + 1e-6  # 各点之间的矢量（第一点不取）  num_instace, num_pts - 1, 2 (0-1, 1-2)
    vector_dic_v2 = (target[:, :-1, :] - target[:, 1:, :]) + 1e-6
    mask = (vector_dic_v2 != 1e-6).max(dim=2)[0].max(dim=1)[0]
    ...
    gt_angle = dot_product / (magnitude_v1 * magnitude_v2)
    gt_angle_rad = torch.abs(torch.acos(gt_angle)[mask][:, :, 0])
    mask_1 = torch.isfinite(gt_angle_rad).all(dim=-1)
    gt_angle_rad[mask_1] = 0.0
    mask_2 = (cls_scores[mask].softmax(dim=1).max(dim=1)[1] != 1)
    gt_angle_rad = gt_angle_rad[mask_2]

    weight_vector_pred_v1 = vector_dic_v1[:, 1:, :]  # num_instace, num_pts - 2, 2
    weight_vector_pred_v2 = vector_dic_v1[:, :-1, :]
    dot_product = torch.sum(weight_vector_pred_v1 * weight_vector_pred_v2, dim=2, keepdim=True)  # num_instace, num_pts, 2
    magnitude_v1 = torch.norm(weight_vector_pred_v1, dim=2, keepdim=True)
    magnitude_v2 = torch.norm(weight_vector_pred_v2, dim=2, keepdim=True)
    pred_angle = (dot_product / (magnitude_v1 * magnitude_v2))
    pred_angle_rad = torch.abs(torch.acos(pred_angle)[mask][:, :, 0])
    pred_angle_rad = pred_angle_rad[mask_2]

    # ischange = torch.nonzero((gt_angle_rad.reshape(-1) < (5 / 180 * 3.14)).int() * (pred_angle_rad.reshape(-1) > (30 / 180 * 3.14)).int()).size(0)
    # global angle_sum
    # global instance_num_
    # angle_sum += ischange
    # instance_num_ += pred_angle_rad.size(0)
    ...
    angle_rad = torch.abs(torch.acos(cos_angle)[mask][:, :, 0])
    mask_1 = torch.isfinite(angle_rad).all(dim=-1)
    angle = angle_rad[mask_1].sum(dim=1)
    mask_2 = (cls_scores[mask][mask_1].softmax(dim=1).max(dim=1)[1] != 1)
    mask_3 = (angle[mask_2] < 19.0)
...
class DiffPtsL1Loss(nn.Module):
    ...
    def forward(self,
        ...
        loss_bbox = self.loss_weight * diff_pts_l1_loss(
            pred, target, cls_scores, labels, weight, reduction=reduction, avg_factor=avg_factor)
        return loss_bbox
...
@mmcv.jit(derivate=True, coderize=True)
def pts_line_loss(pred, target, weight, func_weight):
    """L1 loss.
    ...
    Returns:
        torch.Tensor: Calculated loss
    """
    l1_weight, pts2line_weight, pts2endpts_weight = func_weight[0], func_weight[1], func_weight[2]
    if target.numel() == 0:
        return pred.sum() * 0
    assert pred.size() == target.size()
    weight = weight.reshape(weight.size(0), -1)
    valid_mask = torch.all(weight==1, dim=1)

    pred_start_pt = pred[:, 0, :]
    pred_end_pt = pred[:, -1, :]
    gt_start_pt = target[:, 0,: ]
    gt_end_pt = target[:, -1, :]
    pred_end_pts = torch.stack((pred_start_pt, pred_end_pt), dim=1)
    gt_end_pts = torch.stack((gt_start_pt, gt_end_pt), dim=1)

    loss1 = l1_weight * torch.abs(pred_end_pts - gt_end_pts).sum(-1)
    # loss1 = loss1.reshape(loss1.shape[0],-1)
    # loss1_valid = loss1[valid_mask].sum(-1)
    ...
    AB  = B - A
    AP = P - A
    cross_product = AP[:, :, 0] * AB[:, :, 1] - AP[:, :, 1] * AB[:, :, 0]
    norm_AB= torch.sqrt(AB[:, :, 0] ** 2 + AB[:, :, 1] ** 2)
    distance_to_line = torch.abs(cross_product)/(norm_AB + 1e-8)

    distance_to_A = torch.norm(P - A, dim=-1)
    distance_to_B = torch.norm(P - B, dim=-1)
    distance_to_endpts = distance_to_A + distance_to_B - norm_AB

    # loss = torch.cat([loss1[:,0].unsqueeze(1),distance,loss1[:,1].unsqueeze(1)],dim=-1)
    loss = torch.cat([loss1, pts2line_weight * distance_to_line + pts2endpts_weight * distance_to_endpts], dim=-1)
    loss = loss[valid_mask].sum() / valid_mask.size(0)

    return loss


@LOSSES.register_module()
class PtsLineLoss(nn.Module):
    """L1 loss.
    ...
    def __init__(self, reduction='mean', loss_weight=1.0, func_weight=[1,1,1]):
        super(PtsLineLoss, self).__init__()
        self.reduction = reduction
        self.loss_weight = loss_weight
    ...
    def forward(self,
        ...
        reduction = (
            reduction_override if reduction_override else self.reduction)
        # import pdb;pdb.set_trace()
        loss_bbox = self.loss_weight * pts_line_loss(
            pred, target, weight, self.func_weight)
        return loss_bbox


@MATCH_COST.register_module()
class OrderedPtsLineCost(object):
    """OrderedPtsL1Cost.
     Args:
         weight (int | float, optional): loss_weight
    """

    def __init__(self, weight=1.0, func_weight=[1, 1, 1]):
        self.weight = weight
        self.func_weight = func_weight
    ...
    def __call__(self, bbox_pred, gt_bboxes):
        ...
        num_gts, num_orders, num_pts, num_coords = gt_bboxes.shape
        # import pdb;pdb.set_trace()
        gt_bboxes = gt_bboxes.reshape(-1,num_pts,num_coords)

        start_pt_pred = bbox_pred[:,0,:]
        end_pt_pred = bbox_pred[:,-1,:]
        start_pt_gt = gt_bboxes[:,0,:]
        end_pt_gt = gt_bboxes[:,-1,:]

        pred_end_pts = torch.stack((start_pt_pred,end_pt_pred),dim=1)
        gt_end_pts = torch.stack((start_pt_gt,end_pt_gt),dim=1)
        pred_end_pts = pred_end_pts.view(pred_end_pts.size(0),-1)
        gt_end_pts = gt_end_pts.view(gt_end_pts.size(0),-1)
        l1_cost = torch.cdist(pred_end_pts,gt_end_pts,p=1)

        A = gt_bboxes[:,:-2,:]
        B = gt_bboxes[:,1:-1,:]
        P = bbox_pred[:,1:-1,:]
        A_expanded = A.unsqueeze(0).expand(P.size(0),-1,-1,-1)
        B_expanded = B.unsqueeze(0).expand(P.size(0),-1,-1,-1)
        P_expanded = P.unsqueeze(1).expand(-1,A.size(0),-1,-1)
        AB = B_expanded - A_expanded
        AP = P_expanded - A_expanded
        cross_product = AP[...,0]*AB[...,1] - AP[...,1]*AB[...,0]
        norm_AB= torch.sqrt(AB[...,0]**2 + AB[...,1]**2)
        distance_to_line = torch.abs(cross_product)/(norm_AB + 1e-8)
        distance_to_line = distance_to_line.sum(-1)

        distance_to_A = torch.norm(P_expanded - A_expanded, dim=-1)
        distance_to_B = torch.norm(P_expanded - B_expanded, dim=-1)
        distance_to_endpts = distance_to_A + distance_to_B - norm_AB
        distance_to_endpts = distance_to_endpts.sum(-1)

        cost = self.func_weight[0] * l1_cost + self.func_weight[1] * distance_to_line + self.func_weight[2] * distance_to_endpts
