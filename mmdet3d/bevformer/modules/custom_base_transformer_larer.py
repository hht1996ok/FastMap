...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, query, key, query_pos, key_pos, attn_mask=None):
        ...
        query2 = self.multihead_attn(query=self.with_pos_embed(query, query_pos),
                                     ...
                                     value=self.with_pos_embed(key, key_pos), attn_mask=attn_mask)[0]
...
class BEVMaskGenerator(nn.Module):
    def __init__(self, bev_channels, classes, d_model=256, nhead=8, num_encoder_layers=2, dim_feedforward=1024):
        super(BEVMaskGenerator, self).__init__()

        # 线性投影层，将输入通道数映射到 d_model
        self.input_projection = nn.Linear(bev_channels, d_model)
        # 位置编码
        ...
        # 线性投影层，将 d_model 映射到 1
        self.mask_projection = nn.Linear(d_model, classes)

    def forward(self, x):
        # 输入 x的形状为(batch, h, w, c)
        batch_size, c, h, w = x.shape
        # 将 BEV 特征展平为序列形式
        x = x.permute(0, 2, 3, 1).view(batch_size, h * w, c)  # 形状为(batch, h*w, c)
        # 通过线性投影层映射到 d_model
        x = self.input_projection(x)  # 形状为(batch, h*w, d_model)
        # 添加位置编码
        x = self.positional_encoding(x)  # 形状为(batch, h*w, d_model)
        # 通过 Transformer 编码器
        x = self.transformer_encoder(x)  # 形状为(batch, h*w, d_model)
        # 通过线性投影层生成 mask
        mask = self.mask_projection(x)  # 形状为(batch, h*w, 1)
        # 将序列数据重新调整为二维特征图
        mask = mask.view(batch_size, h, w, 3).permute(0, 3, 1, 2)  # 形状为(batch, 3, h, w)
        return mask
